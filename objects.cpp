...
struct SnakePart {
  ...
  void draw() {
    ...
    DrawRectangle(convertArenaToWindowSize(ARENA_WIDTH, this->posX, width),
                  ...
                  convertArenaToWindowSize(ARENA_WIDTH, 1, width) - 0.1,
                  convertArenaToWindowSize(ARENA_LENGTH, 1, height) - 0.1,
                  BLACK);
  }
...
struct Apple {
  ...
  void draw(vector<SnakePart> player) {
    ...
    DrawRectangle(convertArenaToWindowSize(ARENA_WIDTH, this->posX, width),
                  convertArenaToWindowSize(ARENA_LENGTH, this->posY, height),
                  convertArenaToWindowSize(ARENA_WIDTH, 1, width) - 0.25,
                  convertArenaToWindowSize(ARENA_LENGTH, 1, height) - 0.25,
                  RED);
  }
